...
You only need `git` and `podman` installed:

```bash
git clone https://github.com/abba23/spotify-adblock.git
cd spotify-adblock
podman run -it --rm -v $(pwd):/src:Z ubuntu:23.10 sh -c 'apt update && apt install -y --no-install-recommends make cargo && make -C /src'
```
...
rm -rv ~/.var/app/com.spotify.Client/{spotify-adblock,libspotifyadblock.so}
```
